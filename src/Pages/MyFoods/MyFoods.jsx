...
const MyFoods = () => {
        ...
        const loadMyFoods = async () => {
            try {
                ...
                if (
                    ...
                    setError(
                        "We couldn't load your foods right now. Please try again.",
                    );
            } finally {
                setIsLoading(false);
            }
    ...
    const myFoods = useMemo(
        () =>
            foods.filter(
                (food) =>
                    getOwnerEmail(food)?.toLowerCase() ===
        ...
        <main className="min-h-screen bg-orange-50/40 pb-16">
            ...
            <section className="bg-orange-500 px-4 py-16 text-center text-white sm:py-20">
                ...
                <p className="mx-auto mt-3 max-w-xl text-orange-100">
                    Manage the foods you have added to Hidden Pearl.
                </p>
            </section>
                ...
                {!isLoading && !error && myFoods.length === 0 && (
                    <div className="rounded-2xl bg-white px-6 py-14 text-center shadow-sm">
                        ...
                        <p className="mt-2 text-gray-500">
                            Foods associated with {user?.email} will appear
                            here.
                        </p>
                    </div>
                )}
                            ...
                            <tbody>
                                {myFoods.map((food) => (
                                    ...
                                    >
                                        ...
                                        <td>{food.category || "Food"}</td>
                                        <td className="font-semibold">
                                            ৳{food.price ?? "—"}
                                        </td>
                                        <td>{getQuantity(food)}</td>
                                        <td className="text-right">
                                            <Link
                                                ...
                                                className="btn btn-sm border-0 bg-orange-500 text-white hover:bg-orange-600"
                                            >
                                                Update
                                            </Link>
                                        </td>
